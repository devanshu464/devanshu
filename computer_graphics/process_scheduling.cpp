...
struct p process[25];
struct process p[15];
float basic=36.0,col[5][3]={{1.0,0,.55},{1.0,0.5,0},{.5,.5,.2},{1,.84,.5},{1,.6,.2}};
bool doneInput=false;
char line1[] = {"EAST WEST INSITUTE OF TECHNOLOGY"};
char line2[] = {"Computer Science Department"};
char line3[] = {"   Computer Graphics project"};
char line4[] = {"Topic: Process Scheduling Algorithm"};
char line5[] = {"Devanshu kumar	 : 1EW16CS039"};
char line6[] = {"Ganesh astagi : 1EW16CS047"};
...
//  strin("PRESS R or r TO RESET THE PROGRAM",33,15,300);
    ...
    strin("Made by Devanshu & ganesh",25,5,20);
    glColor3f(0,0,0);
}
            ...
            if(np==1)
            {
                ...
                else
                {
                    for(int k=0;k<3;k++) {
                        ...
                        delay();
                        glClear(GL_COLOR_BUFFER_BIT);
                        glClearColor(.8,0.5,.2,.803);
                        backgrnp();
                        delay();
                    }
                    one=1;
                    glClearColor(.5,0.5,.6,.803);
                }
            }
            else if(att>=0)
            {
